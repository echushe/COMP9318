...
def generate_full_dictionary(samples):
    """
...
def train(strategy_instance, parameters, features_used, test_samples):
    ...
    full_dict = generate_full_dictionary(samples)
...
def update_test_samples(target_pos, x_mul_w, selected_dict, test_samples, weights, test_x, test_y):
...
def investigate_train_results( param ):
    ...
    '''
    The target pos is defined by intuition
    This value may be very important

    boundary_up: 2.79604709099
    boundary_down: 4.52338667271

    target_pos        fool success
    -4                11.5 %
    1.0687075092      8.5 %

    '''
    target_pos = -4 # boundary_up - margin

    print(boundary_up)
    print(boundary_down)

    test_samples = samples[len_class_0 + len_class_1 : ]
    update_test_samples(target_pos, x_mul_w[len_class_0 + len_class_1 : ], selected_dict, test_samples, weights, test_x, test_y)
...
def fool_classifier(test_data): ## Please do not change the function defination...
    ...
    investigate_train_results(train(strategy_instance, parameters, 220, test_samples))
